...
function chartPath(input) {
	...
	for (let x=0;x<input.length;x++) {
		...
		if(direction === "U") {
			...
			targetY = currentY - parseInt(path.slice(1));
			for(let i = targetY; i <= currentY; i++) {
				pathTraveled[steps]=[];
				pathTraveled[steps][0] = currentX;
				pathTraveled[steps][1] = i;
				steps = steps + 1;
			...
			targetX = currentX - parseInt(path.slice(1));
			for(let i = targetX; i <= currentX; i++) {
				pathTraveled[steps]=[];
				pathTraveled[steps][1] = currentY;
				pathTraveled[steps][0] = i;
				steps = steps + 1;
...
let firstPathRoute = (chartPath(firstPath));
let secondPathRoute = chartPath(secondPath);
possibles = [];
let counter = 0;
for (const point of firstPathRoute) {
	for (const point2 of secondPathRoute) {
		if(point[0]===point2[0] && point[1]===point2[1]) {
			let label = counter;
			possibles[label] = point;
			counter = counter+1;
		}
	}
}
...
for (const possible of possibles) {
	if((Math.abs(possible[0])+Math.abs(possible[1]))===0){

	} else if(Math.abs(possible[0])+Math.abs(possible[1])<lowest) {
		lowest = Math.abs(possible[0])+Math.abs(possible[1]);
	}
}
console.log(lowest);